...
└── MOD/MinidoracatLangFor42/      # Workshop 上傳根目錄
    ...
    └── Contents/mods/MinidoracatLangFor42/42/  # ← PZ 模組根目錄
        ...
        └── media/
            ...
            │       └── CN/        # 簡體中文翻譯（CH 完全鏡像）
            ├── maps/Riverside, KY/  # 地圖漢化（出生點 + 世界地圖標籤 + 中文街道名稱）
...
| 中文街道名稱 | `MOD_ROOT/media/maps/Riverside, KY/streets.xml` | 由 `MapStreets_Flx.lua` 載入，SP/MP 通用 |
...
| Mod 版本更新 | `mod.info` + `README.md` + `STEAM_DESCRIPTION.md` + `CHANGELOG.md` | 見「版本發布流程」章節 |
...
MOD 的 `media/maps/` 下**只能有 `Riverside, KY/`**，絕對不能有 `Muldraugh, KY/` 目錄。

**原因**：PZ 引擎看到 MOD 的 `Muldraugh, KY/map.info` 會將 MOD 當作地圖資料來源。
由於我們的 MOD 沒有 `.lotheader` 建築檔，PZ 會找不到建築物資料，導致 **SP 新開檔案建築物不載入**。

```
maps/
└── Riverside, KY/           ← 唯一允許的地圖目錄
    ├── map.info              ← Workshop 上傳需要
    ├── streets.xml           ← 中文街道名稱（全地圖，由 MapStreets_Flx.lua 載入）
    ├── spawnpoints.lua       ← 出生點定義
    ├── spawnSelectImagePyramid.zip  ← 中文地圖圖片
    ├── thumb.png             ← 地圖縮圖
    └── worldmap-annotations.lua  ← 中文地圖標籤（MapLabel_* 翻譯鍵）
```

**`MapStreets_Flx.lua` 載入機制**：
1. 先執行原版 `initDefaultStreetData`（載入英文街道）
2. `clearStreetData()` 清除英文
3. `addStreetData('media/maps/Riverside, KY/streets.xml')` 載入中文

**關鍵**：`addStreetData()` 是**疊加**而非覆蓋，所以必須先 `clearStreetData()` 再載入中文版，否則英文和中文路名會重疊顯示。
**不依賴 `getLotDirectories()`**：因為 MP client 的 `getLotDirectories()` 只返回當前遊戲使用的地塊目錄（如 `Muldraugh, KY`），不包含 `Riverside, KY`。

### Sandbox 翻譯規則

PZ Sandbox 選項的翻譯鍵有嚴格的格式區分：

| 鍵後綴 | 用途 | 值格式 | `\n` 行為 |
|--------|------|--------|----------|
| （無） | 短標籤 | 簡短文字（2-6 字） | ❗ 會原樣顯示 `\n` 字串 |
| `_tooltip` | 描述/提示 | 可以較長 | ✅ PZ 會解釋為換行 |
| `_option1` ~ `_optionN` | 下拉選項 | 簡短文字 | ❗ 同標籤 |
| `_help` | 幫助文字 | 可以較長 | ✅ PZ 會解釋為換行 |
...
發布新版本時，**必須詢問使用者版本號**，然後自動更新以下所有檔案：

| 檔案 | 位置 | 格式 |
|------|------|------|
| `mod.info` | `MOD_ROOT/mod.info` 第 5 行 | `modversion={版本號}` |
| `README.md` | 根目錄，MOD 資訊表格 | `\| **Mod 版本** \| {版本號} \|` |
| `README.md` | 根目錄，MOD 資訊表格 | `\| **支援版本** \| Build {PZ版本}+ \|`（若 PZ 版本變更） |
| `STEAM_DESCRIPTION.md` | 根目錄，MOD 資訊清單 | `[*] [b]Mod 版本:[/b] {版本號}` |
| `STEAM_DESCRIPTION.md` | 根目錄，MOD 資訊清單 | `[*] [b]支援版本:[/b] Build {PZ版本}+`（若 PZ 版本變更） |
| `CHANGELOG.md` | 根目錄 | 新增 `## [{版本號}] - {YYYY-MM-DD}` 區塊 |

**版本號格式**：`{PZ版本}-{Mod主版本}.{次版本}.{修訂}`（如 `42.15.1-1.2.1`）

**CHANGELOG 格式**（遵循 [Keep a Changelog](https://keepachangelog.com/zh-TW/1.1.0/)）：
```markdown
## [{版本號}] - {YYYY-MM-DD}
...
- **在 MOD 中建立 `Muldraugh, KY/` 目錄**：會導致 PZ 將 MOD 當作地圖資料來源，造成 SP 建築物不載入。中文 `streets.xml` 必須放在 `Riverside, KY/`，由 `MapStreets_Flx.lua` 用固定路徑載入
- **Sandbox 標籤放描述文字**：`Sandbox_XXX` 鍵（無後綴）只能放短標籤，PZ 不會解釋 label 中的 `\n`，會原樣顯示且版面跟版
- **`addStreetData()` 當作覆蓋使用**：它是疊加不是覆蓋，必須先 `clearStreetData()` 再載入中文版
...
| `MapStreets_Flx.lua` | `ISUI/Maps/` | 29 | ✅ _orig | 中文街道名稱載入（固定路徑 `Riverside, KY/streets.xml`，不依賴 `getLotDirectories()`） |
