...
> **2026-07-29 二次更新：B 表大部分已修補完成。** 移入 A 表的有——**B12 → A20**（Brush Tool 視窗）、**B15-c/d/e、B16、B18、B19 → A18**（視窗自建選單接線）、**B9 殘餘對話框標題＋B17＋B20＋B21 → A19**（`ISTextBox`/`ISModalDialog` 建構層攔截）；B9 的 tooltip 三條（`Zone not valid`/`Building not valid`/`Tile params:`）由 A18 擴充 walker 涵蓋。
