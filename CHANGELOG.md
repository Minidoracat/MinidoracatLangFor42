...
- 審查歷程：Claude 端獨立驗證（13 項 API 契約全部對 42.20.2 反編譯快照與原版原始碼逐條核對）＋ codex `review-plus` 兩輪 ＋ 第三方交叉檢視，共修掉 8 項。其中兩次推翻了先前自己的結論——「伺服器不該寫加成表，否則會與 client 分岔」被「多人遊戲的經驗是伺服器權威」證偽；「連點節流已完成」被「同一玩家可開多個面板」證偽。兩次的錯誤結論與推翻它的證據都留在 `HARDCODE_REGISTRY.md` A12 與開發筆記中，避免後人照舊結論改回去。
