...
- **修復 1.26.0 更新後文字與介面行距變大、選單互相重疊**：換新字型時少數特殊符號太高，把整個介面的行高撐大約兩成。現在行距恢復成更新前的樣子，新字型的數字對齊維持不變。
  > 技術要點：`AngelCodeFont.parseChar` 以全部字形的 `max(yoffset+height)` 當行高、不讀 `common lineHeight`；
  > 〱〲 等字形超出行格使 1x Small/Medium/Large 由 20/25/30 變 24/30/37。`gen_fonts.py` 改把字形裁回 `[0, lineHeight)`。
  > 實機驗證（`getFontHeight`）：1.25.1＝20/25/30、1.26.0＝24/30/37、修正後＝20/25/30。
- **修復部分玩家更新後選單文字變成亂碼**：字型檔改用本包專屬檔名，不再和其他同樣使用中文字型的模組互相蓋檔。
  > 技術要點：圖集由 `zomboid{Small,Medium,Large}CN*` 改名為 `zomboid{…}CatLang*`（CH／CN／根層），三份 `fonts.txt` 同步。
  > PZ 依相對路徑逐檔覆寫，舊檔名與上游／其他中文 MOD 相同；1.26.0 圖集排版改變後，`.fnt` 座標與他人 `.png` 混搭即成亂碼（推測，未取得玩家 MOD 清單實證）。
...
- 修正藏寶圖把超市名稱顯示成斧頭提示的錯譯，繁中與簡中皆恢復正確店名。
- 修正兩則羅斯伍德售屋廣告的價格、房屋規格與介紹，並同步更新中文傳單圖片，避免圖片與文字顯示不同資料。
- 修正部分廣播台詞與聖詩歌詞，並調整繁中報紙、教學與街名用詞；保留語意正確的跨句翻譯。

## [42.20.4-1.25.0] - 2026-09-11
...
- **修復多人伺服器上肉品名稱顯示英文**（例如「Beef (Prime Cut)」「Steak (Average Cut)」）。
  屠宰動物取得的肉會標示品質等級，但在英文伺服器上整串名稱都是英文。
  **這是遊戲本體的問題，不是翻譯包漏翻**——牛肉、牛排、豬肉、豬排、羊肉、兔肉、鹿肉這七種
  肉，連同三個品質等級的中文，官方翻譯與本包早就都有了，是遊戲組合名稱的流程把它蓋成英文，
  所以**不裝任何模組也會遇到**。官方目前尚未修復，本包改以客戶端還原：現在繁中與簡中都會
  正常顯示成「牛肉 (上等肉塊)」。
...
- **修復多人伺服器上模組物品名稱顯示成內部代碼**（例如「VFX.SourdoughStarter」）。玩家回報
  在伺服器上物品名變成一串英文代碼，單機卻正常。
  **這也是遊戲本體的問題，與翻譯包或那些模組都無關**——官方要求模組把物品名稱全部改走翻譯
  檔之後，伺服器端的載入順序讓模組的翻譯永遠來不及被讀到，於是伺服器把內部代碼當成名稱送給
  玩家。該模組的作者也已公開說明這只發生在多人連線、且非模組本身造成。官方尚未修復，本包
  改以客戶端還原：現在會用你自己的語言把名稱顯示回來。
